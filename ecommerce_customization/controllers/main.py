...
class WebsiteSale(WebsiteSale):
    ...
    def _get_search_domain(self, search, category, attrib_values, search_in_description=True):
        ...
        attribute_value_ids = request.env['product.attribute.value']
        if search:
            for srch in search.split(" "):
                attribute_value_ids = request.env['product.attribute.value'].search([('name','ilike',srch), ('attribute_id.is_use_in_search','=', True)])
                print(attribute_value_ids)
                subdomains = [
                    [('name', 'ilike', srch)],
                    [('product_variant_ids.default_code', 'ilike', srch)],
                    [('attribute_line_ids.value_ids','in', attribute_value_ids.ids)]
                ]
